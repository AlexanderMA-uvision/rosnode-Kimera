...
class PoseGraph {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.nodes.forEach((val) => {
      bufferOffset = PoseGraphNode.serialize(val, buffer, bufferOffset);
    });
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.nodes[i] = PoseGraphNode.deserialize(buffer, bufferOffset)
    }
  ...
  static getMessageSize(object) {
    ...
    object.nodes.forEach((val) => {
      length += PoseGraphNode.getMessageSize(val);
    });
  ...
  static md5sum() {
    ...
    return '13de81448279ba61bd1e3559bdffd178';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new PoseGraph(null);
    ...
    if (msg.nodes !== undefined) {
      ...
      for (let i = 0; i < resolved.nodes.length; ++i) {
        resolved.nodes[i] = PoseGraphNode.Resolve(msg.nodes[i]);
      }
    }
    else {
      resolved.nodes = []
